...
import {  Row,  Col,  Button,  Typography,  Form,  Input,  Radio,  Select,  Space,  Tooltip,  List,  message } from 'antd';
...
const steps = [  {
  ...
  /** * Format duration in HH hr mm mins. *  * @param {int} totalMinutes - Duration in minutes. * @returns {String} - Formated hour and mintues. */

const formatDuration = (totalMinutes) => {
  const duration = parseInt(totalMinutes);
  const HH = Math.floor(duration / 60);
  const mm = duration % 60;  if (HH === 0 && mm === 0) {
    return 'Immediate';
  } else if (HH === 0) {
    return mm +  ' mins';
  } else if (mm === 0) {
